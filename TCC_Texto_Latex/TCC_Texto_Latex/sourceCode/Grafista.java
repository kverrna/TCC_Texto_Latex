...
public class Grafista extends Agent {
	...
	protected void setup()
	{
		...
		System.out.println("Iniciandobusca.....\n"
				+ "-----------------\n");
		addBehaviour(newFindPatterns(this,"C:/ATIVOS/MRVE3SA.csv"));
	}
	...
	public class FindPatterns extends OneShotBehaviour
	{
		...
		publicvoid action() {
			 ...
			 Pattern testPattern;
		        String result = "";
		ArrayList<CandleStick> list = newArrayList<CandleStick>();
		ArrayList<CandleStick>listResult = newArrayList<CandleStick>();
		LoadCandleStickForCSVFile test = newLoadCandleStickForCSVFile(path);
		...
		JOptionPane.showMessageDialog(null, "ListREsult:" + listResult.size());
		JOptionPane.showMessageDialog(null, "List:" + list.size());
		if(listResult.size()>0) System.out.println("Encontrei o padraonasseguintesdadas:...");
		elseSystem.out.println("Para  ativonaoencontrei o padraobuscado  =(");
